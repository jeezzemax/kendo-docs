...
meta_title: Migrating from Telerik Extensions for ASP.NET MVC to UI for ASP.NET MVC
meta_description: What’s the difference between Telerik Extensions for ASP.NET MVC and UI for ASP.NET MVC, learn how to migrate, handle data-binding, set stylesheets.
...
**UI for ASP.NET MVC** contains many differences from Telerik Extensions for ASP.NET MVC. Some of these will be breaking changes.
...
Telerik Extensions for ASP.NET MVC to **UI for ASP.NET MVC**.

Keep in mind that due to the larger changes in terms of assembly name and namespaces, both the Telerik Extensions for ASP.NET MVC and **UI for ASP.NET MVC**
...
Below you will find the major changes coming from **UI for ASP.NET MVC**. Each change marks a difference between Kendo UI and the Telerik Extensions for ASP.NET MVC.
...
With the Telerik Extensions for ASP.NET MVC the assembly that is included with each project is Telerik.Web.Mvc.dll. As **UI for ASP.NET MVC** is not the same binaries, the new Kendo.Mvc.dll assembly will have to be referenced in your project.

### Namespace

_The change_: The root namespace of **UI for ASP.NET MVC** is *Kendo.Mvc.UI*

The root namespace for the Telerik Extensions for ASP.NET MVC has been Telerik.Web.Mvc.
With **UI for ASP.NET MVC** the namespace will be different, namely Kendo.Mvc.UI.
...
. In order to take use of **UI for ASP.NET MVC** an import statement importing Kendo.Mvc.UI will be needed instead.

### ASP.NET MVC version support

_The change_: **UI for ASP.NET MVC** supports ASP.NET MVC 3 and 4

Previously the Telerik Extensions offer support for ASP.NET 1, 2, and 3.
With **UI for ASP.NET MVC** the versions of ASP.NET MVC that are supported are 3 and 4.
...
In order to take use of **UI for ASP.NET MVC** all projects utilizing ASP.NET 1 or 2 will have to upgrade to 3 or 4.

### ScriptRegistrar and StyleSheetRegistrar

_The change_: **UI for ASP.NET MVC** will *not* take use of the ScriptRegistrar and StylesheetRegistar
...
which respectively helped compress and combine scripts and style sheets. With **UI for ASP.NET MVC** these are no longer required, nor used.
The same GZIP compression that both the registrars utilize can be implemented on the web server level.
As for bundling we offer a custom builder tool for the JavaScript files used with **UI for ASP.NET MVC** and additionally ASP.NET 4.5 offers built-in bundling.

### Localization

_The change_: Currently **UI for ASP.NET MVC** is localized via satellite assemblies.

The Telerik Extensions for ASP.NET MVC are localized via RESX files whereas **UI for ASP.NET MVC** offers satellite assemblies. Individual messages can be easily
...
_The change_: Due to the usage of the Kendo UI DataSource component, Ajax binding in **UI for ASP.NET MVC** is done in the following way (Grid component used as an example):
...
_The change_: The JavaScript files associated with **UI for ASP.NET MVC** are the files associated with the Kendo UI widgets (
...
With **UI for ASP.NET MVC** the bare minimum is kendo.web.min.js, which contains Kendo UI-specific JavaScript,
and kendo.aspnetmvc.min.js, which contains the MVC-specific parts of **UI for ASP.NET MVC**.
...
This means that these new JavaScript files have to be added to the project in order for **UI for ASP.NET MVC** to function.
...
With **UI for ASP.NET MVC** the second approach, using the server-side code block within the WebFroms ViewEngine, is no longer supported.
...
However, with **UI for ASP.NET MVC** "ClientEvents" has just become "Events" so the statement above will now be:
...
_The change_: **UI for ASP.NET MVC** will not have the prefix of "On" in front of each of the client-events e.g. "OnChange" is now "Change"

When subscribing to client-side events the Telerik Extensions for ASP.NET MVC used names such as OnLoad, OnChange, OnDataBound etc., but all "On" prefixes are removed with **UI for ASP.NET MVC**. So all event subscriptions will have to remove the "On" prefix as follows:
...
_The change_: In an attempt to limit event bubbling in order to optimize the components, **UI for ASP.NET MVC** utilize Kendo events instead of jQuery events.
...
Kendo UI utilizes its own type of events, which do not create the same event bubbling effect, which are utilized within **UI for ASP.NET MVC**.
...
_The change_: **UI for ASP.NET MVC** utilizes a similar approach to traditional Kendo UI when retrieving the component’s client-side object: `$("#foo").data("kendoGrid")` (with the Grid as an example).
...
However, with **UI for ASP.NET MVC** the proper way to grab the client-side object is:
...
_The change_: With **UI for ASP.NET MVC** binding to an event on the client-side looks like:
...
However, with the usage of Kendo UI events this will no longer work with **UI for ASP.NET MVC**.
...
By default, **UI for ASP.NET MVC** uses the Kendo UI stylesheets.
...
In some cases, developers that migrate from Telerik Extensions for ASP.NET MVC to **UI for ASP.NET MVC**, may want to preserve the previous look of the components.
...
The latter are located in the **wrappers/aspnetmvc/LegacyThemes** folder of the **UI for ASP.NET MVC** distribution package.
